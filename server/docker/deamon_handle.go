...
func explainServerCommand(docker_item *yamlz.DockerItem) func() {
	...
	return func() {
		...
		step = frame
		// 服务检查
		check_info, _ := syscmd.BashCmd(docker_item.CheckCmd, true)
		...
		data, _ := syscmd.BashCmd(docker_item.RestartBefore, true)
		...
		// 重启 监听的服务
		data, err := syscmd.BashCmd(docker_item.Restart, true)
		if err == nil {
